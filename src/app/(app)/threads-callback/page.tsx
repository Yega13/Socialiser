...
import { exchangeThreadsCode } from "./actions";
  ...
  useEffect(() => {
    async function handleCallback() {
      ...
      if (error || !code || !userId) {
        setStatus("Threads authorization failed.");
        setTimeout(() => router.push("/dashboard?error=threads_auth_failed"), 2000);
        return;
      }

      try {
        setStatus("Exchanging token...");
        const redirectUri = `${window.location.origin}/threads-callback`;
        const result = await exchangeThreadsCode(code, redirectUri);

        if (!result.success) {
          setStatus(`Error: ${result.error}`);
          setTimeout(() => router.push("/dashboard?error=threads_failed"), 4000);
          return;
        }
          ...
          .upsert(
            {
              user_id: userId,
              platform: "threads",
              platform_username: result.threads_username ?? null,
              platform_user_id: result.threads_user_id ?? null,
        ...
        router.push("/dashboard?connected=threads");
      } catch (err) {
  ...
  return (
    <div className="flex items-center justify-center min-h-[50vh]">
      <div className="text-center space-y-4">
        <div className="w-8 h-8 border-2 border-[#0A0A0A] border-t-[#000000] animate-spin mx-auto" />
