    ...
    for (const platformId of post.platforms as string[]) {
      ...
      if (platformId === "youtube") {
        ...
        try {
          ...
          if (!ytRes.ok) {
            ...
            results[platformId] = {
              success: false,
              error: errData?.error?.message ?? `YouTube error ${ytRes.status}`,
            };
          } else {
      ...
      if (platformId === "instagram") {
        ...
        if (!conn.platform_user_id) {
          results[platformId] = { success: false, error: "Instagram account ID missing" };
          continue;
        }
        ...
        for (let i = 0; i < (post.media_urls as string[]).length; i++) {
          ...
          if (pathMatch) {
            const { data: signedData } = await supabase.storage
              .from("media")
              .createSignedUrl(pathMatch[1], 3600);
            items.push({ url: signedData?.signedUrl || publicUrl, isVideo });
          } else {
